...
start "Entretien chaudiere - serveur web V 0.26" /min /D "%~dp0build\web" "%PYTHON%" -m http.server %PORT% --bind 127.0.0.1
timeout /t 2 >nul
echo.
echo Etape 3/3 - Ouverture de Chrome...
start "" "http://127.0.0.1:%PORT%/?v=0.26-%RANDOM%"
echo.
echo Si Chrome ne s'ouvre pas, copie cette adresse:
echo http://127.0.0.1:%PORT%/?v=0.26
echo.
